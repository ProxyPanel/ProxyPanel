    ...
    <div class="page-content container">
        <div class="panel">
            ...
            <div class="panel-body">
                <form action="/admin/editGroup" method="post" enctype="multipart/form-data" class="form-horizontal"
                        role="form" onsubmit="return Submit()">
                    <div class="form-group row">
                        <label class="col-form-label col-md-1">分组名称</label>
                        <input type="text" class="form-control col-md-5" name="name" value="{{$group->name}}" id="name"
                                autofocus required/>
                    </div>
                    <div class="form-group row">
                        ...
                        <select data-plugin="selectpicker" data-style="btn-outline btn-primary"
                            ...
                            @if(!$levelList->isEmpty())
                                @foreach($levelList as $level)
                                    <option
                                            value="{{$level->level}}" {{$group->level == $level->level ? 'selected' : ''}}>{{$level->level_name}}</option>
                                @endforeach
                            @endif
                        </select>
