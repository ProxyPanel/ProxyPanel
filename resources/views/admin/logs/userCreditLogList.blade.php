            ...
            <div class="panel-body">
                <div class="form-row">
                    ...
                    <div class="form-group col-lg-2 col-sm-6 btn-group">
                        ...
                        <a href="/admin/userCreditLogList" class="btn btn-danger">重 置</a>
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <td>
                                @if(empty($vo->user))
                                    【账号已删除】
                                @else
                                    <a href="/admin/userCreditLogList?email={{$vo->user->email}}"> {{$vo->user->email}} </a>
                                @endif
                            </td>
                            <td> {{$vo->order_id}} </td>
                            <td> {{$vo->before}} </td>
...
@section('script')
    ...
    <script type="text/javascript">
      ...
      // 搜索
      function Search() {
        window.location.href = '/admin/userCreditLogList?email=' + $('#email').val();
      }
    </script>
@endsection
