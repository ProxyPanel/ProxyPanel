                ...
                <div class="form-row">
                    <div class="form-group col-lg-2 col-sm-4">
                        <input type="number" class="form-control" name="user_id" id="user_id"
                                value="{{Request::get('user_id')}}" placeholder="用户ID"/>
                    </div>
                    ...
                    <div class="form-group col-lg-6 col-sm-12">
                        <div class="input-group input-daterange" data-plugin="datepicker">
                            ...
                            <input type="text" class="form-control" name="start" id="start"
                                    value="{{Request::get('startTime')}}" placeholder="{{date("Y-m-d")}}"/>
                            <div class="input-group-prepend">
                        ...
                        <tr>
                            ...
                            <td>
                                @if(empty($vo->user))
                                    【账号已删除】
                                @else
                                    <a href="/admin/userList?id={{$vo->user->id}}"
                                            target="_blank"> {{$vo->user->email}} </a>
                                @endif
                            </td>
...
@section('script')
    ...
    <script type="text/javascript">
      ...
      // 搜索
      function Search() {
        window.location.href = '/admin/trafficLog' + '?port=' + $('#port').val() + '&user_id=' + $('#user_id').val() +
            '&email=' + $('#email').val() + '&nodeId=' + $('#nodeId option:selected').val() + '&startTime=' +
