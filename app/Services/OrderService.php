...
class OrderService {
	...
	private function setPrepaidPlan(): bool {
		...
		return self::$order->save()
		       && self::$user->update([
				'expired_at' => date('Y-m-d', strtotime(self::$user->expired_at." +".self::$goods->days." days"))
			]);
	}
	...
	private function activatePlan(): bool {
		...
		if($ret){
			return Helpers::addUserTrafficModifyLog(self::$order->user_id, self::$order->id, $oldData,
				self::$user->transfer_enable, '【'.self::$order->pay_way.'】加上用户购买的套餐流量');
		}
	 ...
	 * @return bool
	 */
	private function addReferralLog($inviteeId, $inviterId, $oid, $amount, $commission): bool {
		$log = new ReferralLog();
